<x-layout>
    ...
    @include('.components.Category.categoriesEdit')
    @include('.components.Category.categoriesAdd')
    @include('.components.Category.categoriesDelete')
        ...
        <div class="table-data">
            <div class="d-flex justify-content-between align-items-center flex-wrap p-3">
                ...
                <div class="serach_field-area d-flex align-items-center justify-content-between">
                    ...
                    <button class="btn btn-sm btn-info" data-toggle="modal" data-target="#addCategoryModal" value="">Add Category</button>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <button class="btn btn-sm btn-danger" data-toggle="modal" data-target="#deleteCategoryModal{{ $category->id }}" value="">Delete</button>
                        ...
                        @else
                        <tr>
                            <td colspan="9" class="text-center">There is no type data.</td>
                        </tr>
                        @endif
                    </tbody>
                </table>
...
<script src="{{ asset('assets/js/main.js') }}">
    ...
    $(document).ready(function() {
        ...
        $(document).on("click", ".createCategory", function(e) {
            ...
            $.ajax({
                url: "{{ route('category.add') }}", // Corrected URL using the route() helper
                method: "post",
                data: formData,
                contentType: false,
                processData: false,
                success: function(response) {
                    if (response.status === "success") {
                        ...
                        $("#categoryTable").load(location.href + " #categoryTable"); // Reload the table
                    } else {
        ...
        $(document).on("click", ".editCategory", function(e) {
            ...
            $.ajax({
                url: "/categories/edit/" + categoryID, // Adjust this URL based on your route
                method: "GET",
                success: function(response) {
                    if (response.status === "success") {
                        // Pre-fill the form with the existing data
                        $('#editCategoryForm #categoryName').val(response.category.name);
                        $('#editCategoryForm #categorySlug').val(response.category.slug);
                        $('#editCategoryForm #categoryDescription').val(response.category.description);
                        $('#editCategoryModal').modal('show');
                    } else {
                        alert("Failed to retrieve category data.");
                    }
                },
                error: function(xhr) {
                    alert("An error occurred while retrieving category data.");
                }
            });
        });
        ...
        $(document).on("click", ".updateCategory", function(e) {
            ...
            let formData = new FormData(form);
            let categoryID = $("#editCategoryForm").data("id");

            $.ajax({
                url: "/categories/update/" + categoryID, // Adjust this URL based on your route
                method: "POST",
                data: formData,
                contentType: false,
                processData: false,
        ...
        $(document).on("click", ".deleteCategory", function(e) {
            ...
            $.ajax({
                url: "/categories/delete/" + categoryID, // Adjust this URL based on your route
                method: "DELETE",
                data: {
                    _token: "{{ csrf_token() }}"
                },
                success: function(response) {
                    if (response.status === "success") {
                        ...
                        $("#deleteCategoryModal" + categoryID).modal("hide"); // Close modal
                    } else {
